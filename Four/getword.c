...
extern int BACKSLASH_FLAG_SQUIGGLE;
			...
			switch( iochar ) { // set up a switch statment to handle a few particular cases
				case NEWLINE :
					if ( charCount == 0 ){ // if we haven't seen a character before continue the while loop to get the next character from the input stream
						BACKSLASH_FLAG = 0; // todo Check if scenario <\\n\n\n& vs <\\n\n\& is handled correctly
					    continue;
					}
				...
				//case SQUIGLE:
					// find charCount if it == 0; treat tilda as string; set a specific flag
					// charCount > 0 : append tilda to w*++
			    ...
			    case SQUIGLE:
			        BACKSLASH_FLAG_SQUIGGLE = 1;
                    *w++ = iochar;
                    charCount++;
                    continue;

//				case DOLLARSIGN:
//					BACKSLASH_FLAG_DOLLARSIGN = 1;
//
...
//				// todo is there a next character? if not do below, if so don't do below and handle in parse
